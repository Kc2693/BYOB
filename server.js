...
app.post('/api/v1/exercises', checkAdmin, (request, response) => {
  ...
  for (let requiredParameter of ['exercise', 'level', 'method', 
    'upper_lower_core', 'joint', 'muscle_group_id']) {
...
app.post('/api/v1/muscle-groups', checkAdmin, (request, response) => {
  ...
  for (let requiredParameter of ['muscle_group', 
    'targeted_area', 'train_with']) {
...
function checkAdmin(request, response, next) {
  ...
  if (!token) {
    ...
    try {
      ...
      turingEmail? next() : response.status(403)
...
app.post('/authenticate', (request, response) => {
  ...
  if (email && appName) {
    ...
    response.status(422)
      .json({message: 'Invalid request, must supply email and appName'});
  }
});
...
app.patch('/api/v1/muscle-groups/:id', checkAdmin, (request, response) => {
  database('muscle_groups').where('id', request.params.id).update(request.body)
    .then(() => response.status(200)
      .json({message: `Updated muscle group`}))
    .catch(error => response.status(500).json({error}));
});
...
app.delete('/api/v1/exercises', checkAdmin, (request, response) => {
  ...
  database('exercises').where('id', id).del()
    .then(() => response.status(200)
      .json({ message: `Deleted exercise with id ${request.body.id}`}))
    .catch(error => response.status(500).json({ error }));
});
...
app.delete('/api/v1/muscle-groups', checkAdmin, (request, response) => {
  ...
  database('muscle_groups').where('id', id).del()
    .then(() => response.status(200)
      .json({ message: `Deleted muscle group with id ${request.body.id}`}))
    .catch(error => {
      if (error.code === '23503') {
        return response.status(500)
          .json({ error: 'Please delete associated exercises first'});
      } else {
